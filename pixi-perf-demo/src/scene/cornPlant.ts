...
// 故枯/卷曲叶应比健康叶更小。在前述基础上黄/枯/卷叶各再缩 50%（用户实测仍偏大）：
// healthy 1.0；yellow 0.30、dry 0.35、curled 0.275。
...
interface LeafEntry { ai: number; frame: string; flipX: boolean; sJit: number; wJit: number; lenJit: number; rJit: number; droopK: number; chaos: number;
  fallen: boolean; fx: number; fy: number; fAng: number; } // fallen=落叶(散落根部地面)；fx/fy=相对株高的地面偏移；fAng=躺地朝向(世界系)
...
export class CornPlantView extends Container {
  ...
  private fallenC = new Container(); // 落叶层：反向旋转抵消植株倒伏 → 落叶贴地不随主干转
  ...
  constructor(private atlas: PlantAtlas, private plotId: number, private slotIdx: number) {
    ...
    this.addChild(this.baseA, this.baseB, this.fallenC, this.leavesC); // A 下、B 中、落叶(根部地面)、挂叶上
  }
  ...
  private buildLeafPlan(stage: number, wither: number, stemBlend: number): LeafEntry[] {
    ...
    for (let i = 0; i < cfg.leaves.length && out.length < MAX_LEAVES; i++) {
      ...
      // 落叶：主干露出后，枯/卷叶不再挂在主干上（自然状态这种程度大部分已脱落）→ 散落根部地面
      const fallen = stemVisible && (type === 'dry' || type === 'curled');
      out.push({
        ai: i,
        frame: pick.frame,
        flipX: pick.flip,
        ...
        // —— 落叶散落参数（相对株高的地面偏移 + 躺地朝向）——
        fallen,
        fx: (plantHash(this.plotId, this.slotIdx, 140 + i) - 0.5) * 1.10,    // 横向散布 ±0.55·株高
        fy: 0.02 + plantHash(this.plotId, this.slotIdx, 150 + i) * 0.16,     // 落在根部略靠前(下) 0.02..0.18·株高
        fAng: plantHash(this.plotId, this.slotIdx, 160 + i) * Math.PI * 2,   // 躺地朝向任意（俯视地面落叶各朝各方）
      });
    }
    return out;
  }
  ...
  private applyLeaves(p: CornUpdate, aScale: number): void {
    ...
    this.fallenC.rotation = -p.rotation; // 抵消植株倒伏旋转 → 落叶始终贴地、不随主干转
    for (let k = 0; k < this.plan.length; k++) {
      const e = this.plan[k];
      const ap = cfg.leaves[e.ai];
      let lf = this.leaves[k];
      if (!lf) { lf = new Sprite(); this.leaves[k] = lf; }
      // 落叶进 fallenC(反旋转层)、挂叶进 leavesC；按需重挂父级（计划稳定，极少触发）
      const wantParent = e.fallen ? this.fallenC : this.leavesC;
      if (lf.parent !== wantParent) wantParent.addChild(lf);
      ...
      lf.visible = true;
      if (e.fallen) {
        // —— 落叶：散落根部地面、躺平、绕中心任意朝向（fallenC 已反旋转 → 世界系贴地不随倒伏）——
        lf.anchor.set(0.5, 0.5);
        lf.position.set(e.fx * p.heightPx, e.fy * p.heightPx);
        const fl = ls * 0.82; // 落地略小（贴地/俯视压缩观感）
        lf.scale.set(sgn * fl * e.wJit, fl * e.lenJit);
        lf.rotation = e.fAng;
        continue;
      }
