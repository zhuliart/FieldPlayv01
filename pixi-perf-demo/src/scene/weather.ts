...
export class WeatherOverlay {
  ...
  update(world: World, dtMS: number) {
    ...
    let weedLvl = 0;
    for (const p of world.plots) weedLvl += Math.min(3, p.weeds);
    const weedPct = weedLvl / (world.plots.length * 3);
    const weedTarget = weedPct > 0.02 ? Math.min(0.5, 0.15 + weedPct * 0.6) : 0;
    if (weedTarget > 0) {
