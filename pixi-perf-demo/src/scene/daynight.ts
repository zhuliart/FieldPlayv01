...
export class DayNight {
  ...
  update(tod: number, enabled: boolean, dtMS: number) {
    ...
    this.stars.visible = false; // 去掉星光点（观感不佳）
    if (!enabled) return;

    const dn = dayState(tod);
    // 背景图按 (天气×时段) 自带正确亮度（夜景图本就 ~13% 亮），不再叠 multiply 暗化层。
    // —— 对齐原版 dayOverlay:none；否则与夜景图双重压暗 → 夜间全黑。仅保留暖/冷洗 + 星空 + 车灯。
    this.mul.visible = false;
    // 对齐 H5：dayAdd（暖/冷洗）运行时也是 display:none —— 夜景/天气已是真实美术，叠 add 会错误地提亮/染暖
