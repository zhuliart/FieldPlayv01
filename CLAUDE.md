...
- **移动端方向（勿再加竖屏旋转）**：`stage.ts fitBoard` 始终「不旋转、等比缩放」。曾用竖屏 `rotate(90°)` 把横屏舞台转正，但 **CSS 旋转会破坏触摸坐标**：Pixi 命中测试用画布「轴对齐」的 getBoundingClientRect、完全不认旋转 → 点地块落点全错（手动模式点不动、无反馈）；SVG 路网编辑 getScreenCTM 在祖先旋转下坍塌 → 新增节点全堆到画面最左边(x≈0)。已改为竖屏「只缩放 + 非阻断提示横屏」(`index.html#fp-rotate-hint`，由 fitBoard 据 portrait 显隐)。**勿为"竖屏铺满"再加旋转**。
- **SVG 路网坐标换算用 `getBoundingClientRect`（勿用 `getScreenCTM`）**：`gameHud.ts toPct` 用 SVG 实际屏幕矩形换算百分比。**iOS Safari/WebKit 的 `getScreenCTM` 不计入祖先 HTML 元素的 CSS transform**(`#fp-root` 的 scale) → 即便不旋转，落点也全错、节点堆到画面左上角天空（用户实测）。`getBoundingClientRect` 各浏览器都返回真实渲染矩形(含 scale/letterbox 偏移)，是这类坐标换算的通用正解。Pixi 命中测试本就用 getBoundingClientRect，故手动点地块在 iOS 正常。
