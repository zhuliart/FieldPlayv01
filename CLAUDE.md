...
- **学习大脑 `BrainState`/`robotBrain`/`candidates`/`learnFromTask`**：每步枚举全部可做的事 → 打分 `U=wValue·收益+wUrgency·紧迫−wPower·耗电+类别偏置` → ε-greedy 择优；动作完成后反向更新权重（线性回报回归 + 误差截断 + 权重 clamp[0.05,4]，稳定不发散）。`netWorth=funds+fresh·cropVal(stock)+cropVal(wh)`，目标=最大化它。**优先级不再写死、从学习权重涌现**；距离/载重进入 power 特征 → 就近作业、空载多跑、就近清货更省电。仅保留「电量<12 强制充电」反射兜底。HUD「探索」改读 `brain.eps`（随经验衰减）。
  - **回报口径（关键，改前必读）**：`learnFromTask` 把任务分两类。**养护/投资类**(`plant/till/buyseed/buyres/clear` + `water/fert/drain/cover/weed/weedm`)收益在**未来收成**(作物存活/生长/护苗)、当下 netWorth 不升甚至降→ 用其**预期价值 value 作正向塑形回报**(`max(0.4,value)−小额耗电`)；**否则**(收/卖/入库/充电)用**净资产增量−耗电**。**为什么**：若养护类也按净资产增量算，浇水不涨净资产只耗电→`kind['water']` 被学到 −3 → 机器人【晴天该浇也不浇】(用户实测)；同理会撂荒不种地。养护类只在 `needWater/needFert/天气` 门控满足时才进候选 → 正向塑形不会过量。**勿把养护类移出这个 invest 集**。
  - **存档版本**：reward 语义变更也要 `BRAIN_SAVE_V +1`（养护负偏置从 −3 自愈要 ~200 步、太慢 → 作废旧档让修正立即生效）。现 v3。
...
### 裸眼 3D · 破框展示模式（`scene/naked3d.ts`，按 **P** 进/出；大屏展示用）
> 给超大屏「裸眼 3D」展示用。经典**破框（break-frame）**戏法：银色相框=一扇「窗」，留框内的内容在玻璃后；画在框**之上**、压过银边内沿的前景大植株冲到玻璃**前**、朝观众弹出。背景=游戏本体（框内即当前农场画面）。**已弃**早期"立体沙盘 CSS 倾斜+视差"展示模式（`core/presentation.ts` 已删，勿找回）。
- **素材**（`assets/newbg/`，用户提供）：`frame.png`（透明中心银框，铺满舞台）+ `frame_out_plant1.png`（酸模）/`frame_out_plant2.png`（蛇莓）两株**专用静态成熟图**。另有备选 `frame2.png`（暂未启用）。
- **结构**：`main.ts` 把 `naked3d.frameLayer`（银框）+ `naked3d.heroLayer`（破框株）加到 `app.stage` 顶（z 序：游戏 → 银框 → 破框株）。纯叠加层，关闭即隐藏、对游戏零影响。进入自动隐藏 HUD、退出恢复；左上角分辨率角标。
- **破框株放置（三条铁律，均用户多轮校准，勿回退）**：① **要出框**——压住银边**内沿**破框扑出（叶尖进入银边带），**不是缩在框内**；② **不超外框**——整株包围盒留在舞台 `0..1280×0..720`（画框**外**边界）以内、**不被屏幕边缘裁切**（裁切=「露陷」、破 3D 感）；③ **尺寸偏小**（`heightPx` 左 0.38×STAGE_H / 右 0.24×STAGE_H，根 `by≈0.99×STAGE_H`、`cx` 左 0.118 / 右 0.862）。改位置/大小后务必复核包围盒仍在舞台内（数据校验 `getBounds`）。
- **动效（极简，用户要求）**：**不做生长动画**（静态成熟株）；只「**轻微、偶尔的随风摆**」——阵风包络（两条慢正弦相乘→多数时间近静止、偶尔一阵）×颤动，绕**根锚点**轻旋（不移根）。+ 接地软阴影（随 `field.shadowAlpha` 实时浓淡）。
- **单独打光 = 与画面互补的室内顶光（关键，勿改回 relight）**：破框株**不**用场景 `relight`，而用 `lerpColor(TOP_COOL 0xcdd9ff, TOP_WARM 0xffd0a0, 1-sceneLum)`——**场景冷（夜）→植株暖、场景暖（昼）→植株冷**，**恒亮**（不乘场景明暗）像展柜射灯，与游戏画面光照刻意区分。`Field` 暴露 `relight`/`shadowAlpha`/`sceneLum` 三个取值器供其取实时光照。
- **不透明（用户要求）**：框与植株 `alpha=1` 全程不透明（去掉早期 env 渐入半透）；进出场即时显隐。框中心透明=`frame.png` 自带 alpha（窗口），不叠额外透明效果。

### 小麦换写实图 + 玉米式枯死/残茬（用户提供新素材，`assets/newwheat/` 源 → `assets/plant_wheat_*` 归一化）
- **生长帧替换**（`scripts/normwheat.py`）：新写实整株图按**旧 fillH 曲线**`[0.21,0.37,0.60,0.83,0.97]`归一化到**统一画布 379×720、底部对齐**，覆盖 `plant_wheat_s1..s5` → **标准作物管线零改动**接入（crossfade/gScale/`CROP_BOTTOM.wheat=0.994` 不变）。阶段映射：`baby→grow→tasselout(抽穗)→marture→marture(金黄成熟)`。源图在 `assets/newwheat/`（不进打包列表、仅留作可编辑源）。
- **玉米式枯死/残茬**（`field.ts` 标准分支小麦特例 + `assets.ts` 图集加载 4 帧 `plant_wheat_{dry,stubble,ear,leaf}`）：① **站立枯死/过熟**（非冻死）→ 用 `b` 槽**交叉淡入 `dry`** 真枯黄株（替代仅 tint 染褐）；冻死仍 marture+冷蓝。② **采收残茬**（`phase==='empty' && !dead`）→ `a` 槽显 `drystubble` 矮留茬（高=满生长参考高、不随采收后 gScale 缩）+ ~30% 株 `b` 落 `ear`(drywheatear)/`leaf`(deadLeave) 躺地散布（`plantHash` 定，旋转随机）。归一化：dry/stubble 同 379×720 底对齐（dry fillH0.97 与 s5 对齐交叉淡入、stubble fillH0.22 矮）；ear/leaf 紧裁、居中锚点供散布。`drytrunk` 暂未用。

### 两株新写实植物接入杂草系统（酸模/蛇莓，`assets/newbg/np{1,2}_*`）
- 用户提供 `newplant-*`(酸模 7 阶段)/`newplant2-*`(蛇莓 6 阶段)。`scripts/normplant.py` 取 4 生长帧(baby→young→before-flower→flower)+`dry` 枯萎帧，按 `relH=0.32+0.68×(i/(N-1))^0.8` 归一化（统一画布、底对齐）→ `np1_0..4`/`np2_0..4`。
- `main.ts WEED_DEFS` 登记两类（field 类，酸模 `sizeH56`/spread mix、蛇莓 `sizeH24`/spread patch，加显式 `withered:true` 标志因末帧 `np?_4` 不含 "withered" 字样）→ **用杂草系统现有模式投放**（田/野/路），自动继承实时色罩(relight)+天气×昼夜定向投影。**注**：这两株的**归一化静态成熟图另存为 `frame_out_plant1/2`** 供裸眼3D破框株用（与杂草系统里的多阶段版是两套用途）。
