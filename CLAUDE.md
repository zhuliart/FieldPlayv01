...
- **野草系统（全 Pixi 原创，H5 无）**：写实 PNG 多类 —— weed_8(毛茛喜水) / 9 / 10 / 11(鬼针草) / potentilla(蛇莓匍匐) / plantain(车前草) / yellowdock(恶性·最大≈番茄)。
  ...
  - **平滑爬升生长 + 阶段交叉淡入（`drawWeed`）**：幼苗极小 `plantH=targetH×(0.02+0.98×Lc^1.15)`（原 0.18 起，约缩到 10%）→ 平滑长大，不再"突然变大"。每株**双 Sprite**(`sprite`/`sprite2`)：`sprite`=当前阶段、`sprite2`=下一阶段，按阶段内进度末 30% 交叉淡入（同作物做法）→ 换阶段形态平滑过渡。两株共用 tint/旋转/位置/zIndex。`sprite2` 与 `sprite` 同建于 `cropLayer`、隐藏时 `visible=false`（不渲染、开销小）。
...
- **letterbox 黑边「边缘采样补全」（`index.html#fp-backdrop` + `main.ts updateBackdrop`）**：舞台固定 16:9、`fitBoard` 等比缩放(不变形/不裁切/HUD 不丢)→ 视口非 16:9 必留黑边。黑边填充几经迭代(纯色/渐变/毛玻璃模糊图都被嫌生硬或有接缝/白边)，**现行=边缘采样渐变**：`main.ts` 每 ~700ms 用 `app.renderer.extract.canvas({target:app.stage, resolution:0.08})` 极低分辨率截图 → 读 顶/中/底 中线像素色 → 设 `#fp-backdrop` 为 `linear-gradient(top,mid,bot)` 铺满整屏。上黑边=场景顶边色、下黑边=底边色 → **与画面边缘同色相接、无接缝/无白边、随昼夜自动变**。`#fp-root` **不要加 box-shadow 白描边**(那正是"上下白边"的元凶)。仅有黑边时启用(`|vw/vh−16/9|>0.02`)，零黑边/extract 失败回落 `#fp-wrap` 渐变兜底。**勿改回 cover 裁切或非等比拉伸**(用户明确要「等比 + 边缘补全」)，也勿改回模糊图(细黑边时像画框、有接缝)。
