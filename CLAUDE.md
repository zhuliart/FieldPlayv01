...
- **⚠️ 仓库体积纪律（2026-06 已清理一次，务必维护）**：tar 是固定文件名的二进制，**每轮重新提交都会在 git 历史里留一份完整副本**——曾累积 70 份 → `.git` 膨胀到 **2.3G**，导致另一台电脑 clone 失败（`curl 56 schannel / early EOF`）。已清理：`git filter-repo --path fieldplay-pixi-dist.tar.gz --invert-paths` 清除全部历史副本，重新加回最新一份 tar，**force-push 覆盖 main + 活跃任务分支**；**实测全量克隆 2.3G→187M**（每分支仅 1~2 份 tar）。
  - **本环境清理操作要点（下次照做）**：① **filter-repo 默认移除 origin**，跑完要 `git remote add origin <原 URL>`（先 `git remote get-url origin` 存好）；② 先把工作区 tar 备份到 scratchpad，清理后再 `cp` 回来提交（filter-repo 会连工作区一起删）；③ **本地 git 代理只允许写 `main` 和当前会话分支**——删除任意分支报 **403**、大包 force-push 报 **413**；但**小的 ref 移动(force-update 到一个已存在的 commit)被允许**。故清理**别的分支**(如旧会话分支)用「**折叠**」：`git push --force origin <已清理commit>:refs/heads/<旧分支>`（无新对象、推送极小、绕开 413/403），其旧膨胀历史即变不可达、克隆不再拉取。
  - **2026-06 实例**：旧会话分支 `claude/sleepy-franklin-upvp0b`（69 份 tar、内容已被活跃分支完全涵盖）即用折叠法指到 main 的 `41996a1`；旧 tip `b4485ae` GitHub 保留 ~90 天可恢复。
  - **注意**：① 历史已改写，所有改写前的旧 clone 须重新 clone（187M 很快）；② 此后每轮仍 +51M，**累积约每 15~20 轮需再清一次**（同法 + 强推，需先征得用户同意，按红线）；③ 想根治 recurrence 需改用 GitHub Releases 附件或 Git LFS，但本开发环境只有 git 代理、无 REST API/`gh`(MCP 也无删分支/建 Release 工具)、git-lfs 未装，**无法在此端自动完成**，须用户侧操作。
