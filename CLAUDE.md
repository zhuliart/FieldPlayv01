...
- **投影 / 环境色罩染（`field.ts`+`main.ts`，Pixi 原创）**：
  ...
  - **填实软影（默认·`SH_MODE='silhouette'`，预烤"膨胀填实+模糊"贴图）**：上一版「直接拿植株 alpha 当剪影」对细叶(玉米/小麦/yellowdock)会散成一把斜丝(叶间空隙透出)。现改为**预烤填实阴影贴图**：`getSilShadow(srcTex)` 用 `shadowRenderer.extract.canvas(new Sprite(srcTex))` 取该帧像素(兼容 atlas/RenderTexture/预制图集**所有来源**，无需判 `.resource`)→ canvas 2D ①一圈偏移叠绘**膨胀填实**(闭合叶间空隙)②`ctx.filter=blur` **软边**(老 Safari 无 filter 则跳过、只膨胀+染色)③`source-in` 染平深色 `SH_SIL_TINT 0x1c241e`(抹掉正面色只留外形)。**按源 `Texture` 引用缓存**(`silShadowCache`，每 crop_stage 只烤一次、半分辨率 `SH_SIL_DS=0.5`，零每帧开销)；`main.ts` `setShadowRenderer(app.renderer)` 注入渲染器。`applyShadow` 用烤图：锚点=植株锚点(根重合)、`scale=(s/DS)·{WIDEN1.12, −FLATTEN0.50}`(**负 scale.y 翻转**使影从根投向右下地面、÷DS 抵消降采样)、`skew.x=SH_SIL_SKEW0.45`(略竖填实、`SH_ANGLE` 同向右下)、`rotation=0`、`tint=白`(色已烤进贴图)、alpha×`SH_SIL_ALPHA_K0.85`。**fallback**：`getSilShadow` 返回 null(渲染器未就绪/extract 失败) 或株高<`SH_SIL_MIN_PX`(22px) → 回退彗星团 `SHADOW_BLOB`，绝不崩。透明度走 `shadowAlpha` 天气×昼夜、方向固定不随昼夜转。`SH_MODE='blob'` 一键回退纯彗星团。`clearSilShadowCache()` 备清理用(纹理按 crop_stage 有限、单例 Field 无需常调)。
...
- **舞台铺满 = 拉伸铺满（`stage.ts apply`，用户最终选定）**：舞台固定 1280×720，`apply()` 用**非等比** `bsX=W/STAGE_W, bsY=H/STAGE_H` → `#fp-root transform:scale(bsX,bsY)` 铺满视口（无黑边/无白边，画面随视口比例轻微变形）。**关键：pan/zoom 与触摸坐标全部分轴处理**——`bsX/bsY` 取代旧的标量 `baseScale`：`clampPan`/`toWorld`/捏合锚点用 `sX=bsX*zoom`、`sY=bsY*zoom`；`#fp-game` 平移 `translate(panX/bsX, panY/bsY)`；`onScale(max(bsX,bsY)*zoom)` 提分辨率。Pixi/SVG 命中用 `getBoundingClientRect`（返回拉伸后真实矩形）→ 落点按轴比例自洽、不偏。**演进史**：曾试 等比+黑边（纯色/渐变/毛玻璃模糊图/边缘采样渐变）均被嫌生硬/有接缝/白边/绿光斑 → 全部弃用，`#fp-backdrop` 已删。**勿改回 cover 裁切**（会裁掉边缘 HUD）。
...
- **尺寸**：`CORN_HEIGHT_K=0.72`(field.ts) 把新图(裁了透明边)视觉拉回接近旧玉米；叶 `LEAF_SCALE=0.74`(cornPlant.ts) 防图集大叶过度铺张。**幼苗期/枯萎主干压低**（用户实测太高）：field.ts 算 `heightPx` 时乘 `seedK`(幼苗 growthCont→0 ×0.05 极矮，出苗≥1 平滑长到 ×1) 与 `witherK`(随 stemBlend 0.55→0.86 渐缩到 ×0.5)；两者作用于 heightPx → 主干/叶/落叶等比同缩、不squash。**回退**：无；要调观感改这两常量或 `corn_config` 挂点/`cornPlant` 的 WITHER_*/STEM_* 阈值。
