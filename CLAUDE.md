...
- **letterbox 黑边「毛玻璃补全」（`index.html#fp-backdrop` + `main.ts updateBackdrop`）**：舞台固定 16:9、`fitBoard` 等比缩放(不变形/不裁切/HUD 不丢)→ 视口非 16:9 必留黑边。黑边不再用纯色/渐变(用户嫌生硬)，而是 `main.ts` 每 ~700ms 用 `app.renderer.extract.canvas({target:app.stage, resolution:0.35})`+`toDataURL(jpeg,0.5)` 截当前场景 → 设为 `#fp-backdrop` 的 `background`(cover + `blur(30px)` + 压暗 + `scale(1.18)`)→ 留白处是同一场景的模糊延续、自然渐隐、无硬切口。仅在有黑边时启用(`|vw/vh − 16/9|>0.02`)，零黑边/extract 失败回落 `#fp-wrap` 渐变兜底。**勿改回 cover 裁切或非等比拉伸**(用户明确选「等比 + 补全渐隐」)。
