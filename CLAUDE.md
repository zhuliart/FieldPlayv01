...
- **一键隐藏全部 UI（纯净田地视图）**：`GameHud.toggleUi()` 切 `this.root`(整层 HUD)+`svgRoad` 显隐；`main.ts` 绑桌面 **T 键** / 移动端 **三连击屏幕**（同区域 ≤500ms 内 3 次 pointerdown，与分散的游戏点击区分）。隐藏时弹一条 1.4s 自动淡出的恢复提示（独立于 HUD，不随其隐藏）。
