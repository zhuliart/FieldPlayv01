...
- **枯萎 vs 残茬（关键：主干只在残茬态露出）**：`CornUpdate.harvested`(field.ts `sl.phase==='empty' && !dead`，即收割/清枯后空置) 驱动 `stemBlend = harvested?1:0`：
  - **生长/枯萎(站立株)** `stemBlend=0`：**始终显完整阶段图**(baseA，不再淡出成秃秆)，枯萎靠 `baseTint` 随 `wither` 渐染枯黄(`ageY=lerp(白,0xcaa658,wither·0.8)`) + 叠加 黄/干/卷叶(挂在株上、`applyLeaves` 非 fallen 分支)；株高不压低(枯萎=正常株型+枯黄+枯叶，非变矮秃秆)。dead 站立株同此路径(死亡色)。
