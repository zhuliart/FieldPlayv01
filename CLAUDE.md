...
- **移动端双击放大（`stage.ts`）**：触摸设备双击画面 → 在 fit 基准缩放上再叠一层 `translate+scale`(2.2×、居中到双击点、平移钳制防露白)；放大态再双击复位。**关键：因 Pixi/SVG 命中都用 getBoundingClientRect，放大+平移后点击坐标自动仍正确 → 放大状态下照样能操作**（无头实测：2.2× 且舞台被平移到 rect.left=-54 时，点击落点仍精确吻合）。捕获阶段 `window` pointerup 检测双击并 `stopPropagation` 吞掉第二击（避免误触发地块操作/加节点；托管/查看模式单击本无操作，最干净；手动/路网编辑仅首击可能触发一次，可接受）。`onScale(baseScale×zoom)` 让画布按放大后像素提分辨率(封顶 2.5，不爆 GPU)。**勿改回 rotate 或 getScreenCTM**，否则放大坐标会再次错位。
